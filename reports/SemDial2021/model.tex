...
\centering
\begin{tikzpicture}[>=latex,text height=1.0ex,text depth=0.25ex]
  \matrix[row sep=0.35cm,column sep=0.4cm] {
  ...
  \node (Y_T) []{$\hat{Y}_T$};&
  \\ % Second line: RNNs
  \node (RNN_0) [rnn]{\small RNN};&
  \node (RNN_1) [rnn]{\small RNN};&
  \node (dots2) [] {$\dots$};  &
  \node (RNN_T) [rnn]{\small RNN};&
  \\ % Third line: Linear layers
  \node (Encoder_0) [encoder]{\small Encoder};&
  \node (Encoder_1) [encoder]{\small Encoder};&
  \node (dots3) [] {$\dots$};  &
  \node (Encoder_T) [encoder]{\small Encoder};&
