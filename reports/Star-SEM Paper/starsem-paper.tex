...
\begin{abstract}
  ...
  % New abstract?
  % In this we investigate whether transformer models can learn to represent phenomena specific to dialogue.
  % Laughter tokens which are not present in BERT pre-trained models can be learned during additional pre-training and fine-tuning on dialogue data. 
  % We use dialogue act recognition as a proxy task and argue that laughter is a useful feature for it.
  % We observe that pre-training and fine-tuning procedures are useful for learning laughter representation, and explore how well the model adapts to the dialogue domain.
...
Laughter can act as a contextual feature for determining the sincerity of an utterance, e.g. to when detecting sarcasm \citep{tepperman2006yeah}.
