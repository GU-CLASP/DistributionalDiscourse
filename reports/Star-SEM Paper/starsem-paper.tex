...
Finally, \citet{chakravarty2019dialog} uses BERT for dialogue act classification for a proprietary domain and achieves promising results, and \citet{ribeiro2019deep} surpass the previous state-of-the-art on generic dialog act recognition for Switchboard and MRDA corpora. In our work we are not attempting to beat the current state-of-the-art due to the different aims of the study.
...
and finally the standard BERT\textsubscript{BASE} model with no addational pre-training.
