...
We investigate whether BERT \citep{devlinBERTPretrainingDeep2018}, a well-know pre-trained language model, is useful for dialogue.
...
We also show that laughs can be helpful to disambiguate a dialogue act.
...
\begin{itemize}
  % related because of the DAR sequence model
  ...
  % related because of the utt encoder model
  ...
  % related because of methodology
...
\begin{itemize}
  \item \citet{cerisaraEffectsUsingWord2vec2017} -- standard pre-trained word embeddings do not help for DAR in any of three tested languages (English, French, Czech)
    % BERT for dialogue (not nesc. DAR)
